...
class PendingServicePage extends Component
{
    use WithPagination;
    ...
    {
        $data['counter'] = Auth::user()->id == 1 ? Service::where('status',0)->when($this->unit , function ($query, $unit) {
        ...
        $data['lists'] = Auth::user()->id == 1 ? Service::where('status',0)->when($this->unit , function ($query, $unit) {
            return $query->where('unit',$unit);
        })->when($this->speciality , function ($query, $speciality) {
            return $query->where('speciality',$speciality);
        })->when($this->regularity , function ($query, $regularity) {
            return $query->where('regularity',$regularity);
        })->paginate($this->perpage) : Service::where('user_id',Auth::user()->id)->where('status',0)->when($this->unit , function ($query, $unit) {
            return $query->where('unit',$unit);
        })->when($this->speciality , function ($query, $speciality) {
            return $query->where('speciality',$speciality);
        })->when($this->regularity , function ($query, $regularity) {
        ...
        return view('livewire.pending-service-page', $data);
    }
}
